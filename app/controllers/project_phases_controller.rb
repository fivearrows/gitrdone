  ...
  def create
    ...
    respond_to do |format|
      if @project_phase.save
        format.html { redirect_to(@project_phase, :notice => 'ProjectPhase was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @project_phase.update_attributes(params[:project_phase])
        format.html { redirect_to(@project_phase, :notice => 'ProjectPhase was successfully updated.') }
        format.xml  { head :ok }
      else
